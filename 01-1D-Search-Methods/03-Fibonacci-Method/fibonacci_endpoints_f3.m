...
results_a_f3 = cell(size(l_values)); % Αποθήκευση ακραίων τιμών a_k για κάθε l
results_b_f3 = cell(size(l_values)); % Αποθήκευση ακραίων τιμών b_k για κάθε l
...
for l_idx = 1:length(l_values)
    ...
    f1k = f3(x1); % Υπολογισμός της συνάρτησης f3
    f2k = f3(x2); % Υπολογισμός της συνάρτησης f3
    k = 1;
    ...
    results_a_f3{l_idx} = ak_values;
    results_b_f3{l_idx} = bk_values;
end
...
for i = 1:length(l_values)
    plot(1:length(results_a_f3{i}), results_a_f3{i}, '-o', 'DisplayName', ['a_k, l = ', num2str(l_values(i))]);
    plot(1:length(results_b_f3{i}), results_b_f3{i}, '-x', 'DisplayName', ['b_k, l = ', num2str(l_values(i))]);
end
xlabel('Δείκτης Επαναλήψεων (k)');
ylabel('Τιμές a_k και b_k');
title('Τιμές a_k και b_k για τη συνάρτηση f3(x) και διάφορα l');
legend show;
grid on;
hold off;
